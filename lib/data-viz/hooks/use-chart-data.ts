...
import type { ChartData, AggregationConfig, ComparisonConfig } from '../core/types';
import { normalizeData } from '../data/formats';
import { aggregate } from '../data/aggregators';
import { comparePeriods } from '../data/comparators';
import { normalizeTimezone, groupByTimezone } from '../time/timezone';
  ...
  return useMemo(() => {
    ...
    try {
      ...
      if (options?.timezone) {
        ...
        if (chartData.metadata.timezone && chartData.metadata.timezone !== timezone) {
          chartData = {
            ...chartData,
