import type { ChartData, ComparisonConfig, TimeSeriesDataPoint } from '../core/types';
import { parseDate } from '../time/date-utils';
import { subYears, subMonths, subDays, isAfter, isBefore } from 'date-fns';

export function comparePeriods(data: ChartData, config: ComparisonConfig): ChartData {
  const { type, baseField, comparisonField, period } = config;
...
export function yearOverYear(data: ChartData, field: string): ChartData {
  ...
  const yearMap = new Map<number, typeof data.data>();
  ...
  for (let i = 1; i < years.length; i++) {
    ...
    for (const currentPoint of currentPoints) {
      ...
      if (matchingPrevious) {
        const currentValue = typeof currentPoint[field] === 'number' ? currentPoint[field] : 0;
        const previousValue = typeof matchingPrevious[field] === 'number' ? matchingPrevious[field] : 0;
  ...
  columns.push(`${field}_previous`, `${field}_change`, `${field}_changePercent`);
...
function monthOverMonth(data: ChartData, field: string): ChartData {
  ...
  for (let i = 1; i < sorted.length; i++) {
    ...
    const monthDiff = (currentTimestamp.getFullYear() - previousTimestamp.getFullYear()) * 12 +
...
function periodOverPeriod(data: ChartData, field: string, period: number): ChartData {
  if (data.data.length === 0) {
    return data;
  }
